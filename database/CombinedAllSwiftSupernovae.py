...
for i in range(5, int(str(date.today())[2:4]) + 1): #gathers a list of years
    end = "-01-01 00:00:00"
    if i < 10:
        year = "200" + str(i) + end
    else:
    ...
    for criterion in criteria: #check for "ASASSN", "AT", "ZTF", "iPTF", "PTF", "SN", "DLT"
        if criterion.lower() in name.lower():
            if not (("bat" in name.lower()) or ("comet" in desc.lower())): #excludes BAT objects, comets 
                AllSwiftSupernovae.append(too)
                print(name)

#removes duplicates since SN and ASSASN will create duplicate entries from my loops.
AllSwiftSupernovaeClean = list(k for k,_ in itertools.groupby(AllSwiftSupernovae))

with open("AllSwiftSupernovae.csv", "w", newline = "", encoding = "utf-8") as wdata: #writing to another csv
    csvwriter = csv.writer(wdata)

    for i in range(len(AllSwiftSupernovaeClean)):
        csvwriter.writerow(AllSwiftSupernovaeClean[i])

print("Successfully Updated AllSwiftSupernovae.csv!")